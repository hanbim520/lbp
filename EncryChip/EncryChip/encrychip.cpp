...
int  GetCheckPWStringValue(char *pwstring_in, char *day4byte, char *altstatus4byte)
{
	...
	DecryptIODataFromChip((U8*)pwstring_in, 14, (unsigned char*)decrypt_buff, &Encrypt);
	//for (i = 0; i < 20; ++i)
...
static void GetBaseTime(LARGE_INTEGER *baseTime)
{
	SYSTEMTIME SysTime;
	FILETIME FileTime;
	...
	baseTime->LowPart = FileTime.dwLowDateTime;
	baseTime->HighPart = FileTime.dwHighDateTime;
	baseTime->QuadPart /= SECS_TO_FT_MULT;
}


void WinGetTimeOfDay(struct timeval *timeVal)
{
	SYSTEMTIME SysTime;
	FILETIME FileTime;
	LARGE_INTEGER Li;
	...
	LARGE_INTEGER g_BaseTime;
	GetBaseTime(&g_BaseTime);
	Li.QuadPart -= g_BaseTime.QuadPart;

	timeVal->tv_sec = Li.LowPart;
	timeVal->tv_usec = SysTime.wMilliseconds;
}
