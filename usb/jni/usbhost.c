                  ...
                  void *client_data)
{
    ...
    LOGD("Created device discovery thread\n");
...
retry:
    ...
    if (fd < 0) {
        ...
        LOGD("usb_device_open open returned %d errno %d\n", fd, errno);
        ...
        if (fd < 0)
            return NULL;
        writeable = 0;
        LOGD("[ usb open read-only %s fd = %d]\n", dev_name, fd);
    }
...
struct usb_device *usb_device_new(const char *dev_name, int fd)
{
    ...
    LOGD("usb_device_new %s fd: %d\n", dev_name, fd);

    if (lseek(fd, 0, SEEK_SET) != 0)
        goto failed;
    length = read(fd, device->desc, sizeof(device->desc));
    LOGD("usb_device_new read returned %d errno %d\n", length, errno);
    if (length < 0)
        goto failed;
...
static int usb_device_reopen_writeable(struct usb_device *device)
{
    ...
    LOGD("usb_device_reopen_writeable failed errno %d\n", errno);
    return 0;
}
...
struct usb_request *usb_request_wait(struct usb_device *dev)
{
    ...
    while (1) {
        ...
        LOGD("USBDEVFS_REAPURB returned %d\n", res);
        if (res < 0) {
            if(errno == EINTR) {
                continue;
            }
